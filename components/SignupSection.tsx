  ...
  return (
    <section id="signup" className="bg-blast py-24 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-3xl px-5 text-center relative">
        <p className="font-comic text-2xl tracking-wide text-white/90 -rotate-2 inline-block">{copy.eyebrow}</p>
        <h2 className="mt-3 font-display text-white text-6xl sm:text-7xl leading-[0.95]">{rich(copy.heading)}</h2>
        <p className="mt-4 text-ink font-medium max-w-xl mx-auto leading-relaxed">{copy.paragraph}</p>
        <SignupForm
          inputId="email-bottom"
          signup={signup}
          buttonLabel={copy.button}
          defaultNote={copy.note}
          large
          center
        />
      </div>
