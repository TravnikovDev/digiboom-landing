...
import { blogIndexPath, blogPath, defaultLocale, type Locale } from "@/i18n/config";
...
export function blogIndexMetadata(locale: Locale, localesWithPosts: Locale[]): Metadata {
  ...
  const languages = Object.fromEntries(localesWithPosts.map((l) => [l, blogIndexPath(l)]));
...
export function blogPostMetadata(post: Post, translations: Partial<Record<Locale, string>>): Metadata {
  const languages = Object.fromEntries(
    (Object.entries(translations) as [Locale, string][]).map(([l, slug]) => [l, blogPath(l, slug)]),
  );
